...
int init_pubs(ros::NodeHandle &n) {
	...
	for (int i=0;i<2;i++) {
		...
		pub_master_pose_raw[i] = n.advertise<geometry_msgs::PoseStamped>( MASTER_POSE_TOPIC(LR[i]), 1);
		pub_master_pose[i] = n.advertise<geometry_msgs::PoseStamped>( MASTER_POSE_RAW_TOPIC(LR[i]), 1);
	}
